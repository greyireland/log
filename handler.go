...
func RotatingFileHandler(filePath string, limit uint, formatter Format, expire time.Duration) (Handler, error) {
	...
	fileWriter := NewAsyncFileWriter(filePath, int64(limit), expire)
	fileWriter.Start()
